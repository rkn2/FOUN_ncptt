...
We develop a data-driven approach for assessing structural vulnerability in adobe structures at Fort Union National Monument (FOUN), integrating correlation analysis, Factor Analysis, and Regularized Regression (Elastic Net) to identify Necessary and Sufficient Conditions (NSCs) affecting structural behavior \cite[]{harris2001building}. 
...
Height Loss emerged as a powerful predictor (Coefficient = 43.0), reflecting the cumulative loss of wall fabric.
\subsubsection{Geometric Validation Sub-study}
To test the hypothesis that geometric slenderness drives degradation, a supplementary analysis was conducted on a subset of 38 walls for which detailed profilometry data was available. The Slenderness Ratio (Height/Width) was calculated from the physical profiles and correlated with the Total Degradation Score. The analysis yielded a Pearson correlation of $r = -0.016$ ($p = 0.925$), indicating no statistically significant relationship between geometric slenderness and overall degradation in this dataset. This null result reinforces the finding that degradation at Fort Union is primarily condition-driven (e.g., material loss, exposure) rather than a function of elastic instability.
