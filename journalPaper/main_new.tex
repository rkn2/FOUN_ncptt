...
Elastic Net regression identified Wall Height and Cap Deterioration as the strongest predictors of overall degradation, alongside Out-of-Plane movement. Wall Height (standardized coefficient 45.6) demonstrates that geometric slenderness governs adobe vulnerability, while Cap Deterioration (58.0) reflects the dependence on water protection at the wall crest.
...
Building upon the insights gained from correlation and Factor Analysis, we employed Regularized Linear Regression (Elastic Net) to identify the most significant predictors of overall degradation. 
Given the limited sample size (n=67) relative to the number of features, standard linear regression is prone to overfitting. 
Elastic Net addresses this by combining L1 (LASSO) and L2 (Ridge) regularization penalties, effectively handling multicollinearity and performing feature selection by shrinking irrelevant coefficients to zero \cite[]{zou2005regularization}.
The model predicts the overall degradation score (Total Scr) based on geometric features and damage indicators.
All predictors were standardized (zero mean, unit variance) prior to modeling, allowing the magnitude of regression coefficients to serve as a direct measure of relative feature importance.
We employed 5-fold cross-validation to assess model performance and ensure stability.

Model performance was robust, achieving a cross-validation R² = 0.80 (±0.06). 
This high predictive accuracy confirms that the Total Score is effectively a linear composite of the observed damage and geometric factors.
Unlike Random Forest, which struggled with overfitting in preliminary trials (CV R² $\approx$ 0.60), Elastic Net provided a stable, generalizable model appropriate for the sample size.
...
The top predictors of degradation were Cap Deterioration (Coefficient = 58.0), Out of Plane movement (55.5), and Wall Height (45.6).
Structural Cracking (29.3) and Cracking at Wall Junction (28.4) also showed significant positive associations.
These results align with the previous findings but with much higher statistical confidence (R² = 0.80).
Wall Height remains a top-tier predictor, reinforcing the conclusion that geometric slenderness is a primary driver of vulnerability.
The strong performance of the linear model confirms that degradation scoring follows a predictable, additive logic where specific defects (caps, leaning, cracks) cumulatively drive the overall condition state.
...
This geometric-only model achieved a cross-validation R² = 0.21 (±0.14). 
...
Height emerged as the single most powerful geometric predictor (Coefficient = 61.8), far exceeding Point Cloud Deviation (-25.4) and Foundation Height (-21.3).
This result is critical: when blind to actual damage conditions, the model relies heavily on Wall Height to estimate risk.
...
Elastic Net analysis identified Wall Height and Cap Deterioration as the strongest predictors of degradation (standardized coefficients 45.6 and 58.0), challenging the assumption that geometric factors are secondary to visible damage. 
A supplemental geometric-only model (R² = 0.21) further confirmed that Wall Height is the dominant intrinsic risk factor.
