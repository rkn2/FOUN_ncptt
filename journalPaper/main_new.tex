...
Wall Height (standardized coefficient 43.0) and Cap Deterioration (54.5) emerged as the dominant predictors, demonstrating that geometric slenderness and water protection are strongly associated with vulnerability, with tall walls exhibiting higher degradation scores independent of visible damage conditions.
...
When blind to actual damage conditions, the model depends heavily on Wall Height to predict degradation scores.
...
Taller walls experience increased vulnerability through multiple interacting mechanisms, each contributing to cumulative structural degradation. The dominant mechanism is wind load vulnerability, where overturning moment from lateral wind pressure scales with height squared ($M \propto h^2$). For a wall twice as tall, the base moment quadruples under equal wind pressure. For a typical wall section at FOUN with thickness $t = 0.60$ m, height $h = 4.46$ m, and adobe density $\rho = 1800$ kg/m³, the critical wind velocity for incipient overturning is governed by moment equilibrium. The stabilizing moment from self-weight ($M_{stab} = \frac{1}{2}\gamma t^2 h$) must exceed the overturning moment ($M_{OT} = \frac{1}{2}qh^3$), where $q$ is the peak dynamic pressure. This equilibrium condition yields a critical wind velocity that decreases with increasing slenderness. For the tallest walls in this dataset ($h/t \approx 8.3$), even moderate wind events (15--20 m/s) generate significant base tensile stresses which unreinforced adobe cannot sustain given its negligible tensile capacity ($<0.1$ MPa).

Buckling susceptibility represents a second critical vulnerability mechanism. Adobe walls, functioning as slender unreinforced masonry elements, are vulnerable to elastic buckling according to Euler's classical column theory. The critical buckling load is defined as $P_{cr} = \frac{\pi^2 EI}{(kL)^2}$. For walls with nominal fixity at the base but free at the top (cantilever-like behavior, $k \approx 2.0$), and assuming an elastic modulus $E \approx 200$--$400$ MPa \citep{Illampas2014}, walls approaching $h/t > 10$ enter the buckling-critical regime under self-weight alone. The dataset's maximum observed slenderness ratio of 8.33 approaches this theoretical threshold. This vulnerability is exacerbated by P-$\delta$ effects, wherein lateral displacement from out-of-plane deflection induces secondary moments that amplify geometric instability even before material strength is exceeded.

Environmental exposure patterns create a third vulnerability pathway. Upper wall portions experience concentrated wind-driven rain without shelter from adjacent structures or roof overhangs. Moisture penetration depth scales with exposure duration and the sorptivity of the earthen matrix (typically 0.5--2.0 mm/min$^{0.5}$). Taller walls present greater surface area for moisture ingress, and the cumulative effect of wetting-drying cycles accelerates erosion through salt crystallization pressures and clay particle dispersion. This moisture concentration at wall crests explains the strong correlation observed between Cap Deterioration and overall degradation scores (coefficient 54.5 in the Elastic Net model).

While self-weight induced compressive stress at the base increases linearly with height ($\sigma = \gamma h$), nominally reaching only $\approx 90$ kPa for the tallest walls (well below typical adobe capacity of 0.8--2.0 MPa), it contributes to vulnerability when combined with geometric imperfections. Stress concentrations arising from settlement or construction discontinuities can amplify these stresses, while simultaneous lateral wind forces reduce effective compressive capacity through axial-flexural interaction.

Foundation interaction effects represent a final pathway through which geometric factors influence degradation. Exposed foundations experience direct impact from precipitation rebounding off ground surfaces, concentrating moisture at the critical wall-foundation interface. Greater exposed foundation height may indicate inadequate initial embedment or progressive soil loss; if the latter, the lower wall courses lose the passive earth pressure that provides lateral support, increasing vulnerability to out-of-plane movement. Furthermore, capillary rise from saturated soils can wick moisture upward into the adobe pore network, potentially reaching 0.5--1.0 m above grade. These mechanisms operate simultaneously and are difficult to separate using cross-sectional data alone.

The statistical finding that Height dominates predictions even in the geometric-only model ($R^2 = 0.17$) provides empirical evidence that these physics-based mechanisms govern degradation independently of the current damage state. Conversely, the low importance scores for specific historical treatments suggest minimal predictive power in this dataset. This pattern likely arises from selection bias (treatments applied to already-degraded walls), treatment degradation over time, or insufficient sample sizes for specific interventions (e.g., $n=4$) to detect causal effects.

To address these findings, future preservation planning should prioritize preventive intervention for walls exhibiting geometric risk factors (high slenderness, exposed foundations) regardless of their current visible damage. Longitudinal monitoring is required to separate mechanism effects: this should include soil moisture measurements, foundation bearing pressure monitoring, and repeated precision LiDAR scanning to quantify settlement rates and treatment durability.
