...
We developed a data-driven framework for identifying damage co-occurrence patterns and intervention priorities in historic adobe at Fort Union National Monument, integrating correlation analysis, Factor Analysis, and regularized regression (Elastic Net) to analyze relationships among degradation conditions.
On-site surveys of 67 wall sections, archival research, and LiDAR scanning provided the empirical basis for the analysis. Factor Analysis isolated three latent patterns of damage co-occurrence (Sill Deterioration, Surface/Lintel Interaction, and Structural Instability) explaining 52.5\% of total variance.
Elastic Net regression was applied to analyze the relationship between individual condition scores and the composite Total Degradation Score. Ridge and Elastic Net significantly outperformed ensemble methods (Random Forest, Gradient Boosting), with mean cross-validated $R^2$ of 0.76 vs. 0.59 ($p<0.001$), indicating the composite score follows predictable additive logic.
Wall Height Loss (standardized coefficient 43.0) and Cap Deterioration (54.5) emerged as the dominant contributors to the composite score, demonstrating that loss of original fabric and water protection are the most heavily weighted factors in the assessment system. However, because the composite score is mathematically derived from these individual conditions, the high $R^2$ reflects the internal consistency of the scoring system rather than independent predictive power. A supplementary validation on a subset of walls ($N=38$) with detailed geometric profiles found no significant correlation between gross dimensional ratios and degradation ($p=0.92$), confirming that condition metrics dominate the scoring system.
Statistically-derived damage patterns were synthesized with Secretary of Interior preservation standards into intervention matrices, providing field managers with a structured decision framework.
...
The present study applies similar data-driven techniques to historic adobe structures at Fort Union, identifying damage patterns and intervention priorities for preservation planning.

\section{Research aim}

We develop a data-driven approach for identifying damage co-occurrence patterns and intervention priorities at Fort Union National Monument (FOUN), integrating correlation analysis, Factor Analysis, and Regularized Regression (Elastic Net). The analysis serves two complementary purposes: (1) identifying latent patterns of co-occurring damage conditions through Factor Analysis, and (2) recovering the implicit weights of the composite degradation scoring system through regression analysis. By quantifying which individual condition scores (e.g., Cap Deterioration, Height Loss) contribute most heavily to the overall assessment rating, the analysis validates and refines the existing scoring protocol, enabling more efficient resource allocation. The quantified relationships among degradation conditions are then translated into prioritized intervention strategies that balance statistical evidence with preservation ethics and structural requirements. 
The approach bridges traditional condition assessment with statistical pattern analysis, providing empirical structure for resource allocation decisions in contexts where multiple damage modes co-occur and interact.
...
The factor name ``Sill Deterioration" reflects that the observed covariance pattern involves sills, which function as water-shedding elements at the base of openings. The statistical association does not establish causality; sill deterioration could facilitate water ingress, or both sill damage and core wetting could result from common causes such as concentrated runoff from the exposed wall top or preferential moisture accumulation at openings. Temporal monitoring would be required to establish causal pathways.
...
This factor structure indicates statistical association between these two forms of damage. 
Several causal pathways could explain this covariance: (1) surface coat failure around openings may expose wooden lintels to moisture, accelerating decay; (2) lintel deflection may induce stresses in surrounding plaster, causing coating cracks; or (3) both may result from moisture concentration at opening soffits due to inadequate flashing or drainage. Factor Analysis cannot distinguish among these mechanisms. 
The observed association has implications for intervention planning, suggesting that lintels and their surrounding surface coats may benefit from coordinated treatment approaches, though confirmatory structural investigation would be required to identify specific failure mechanisms.
...
To explore whether gross geometric dimensions predict degradation, a supplementary analysis was conducted on a subset of 38 walls for which detailed profilometry data was available. The dimensional ratio (Height/Width) was calculated from the physical profiles and correlated with the Total Degradation Score, yielding no significant correlation ($r = -0.016$, $p = 0.925$). However, this finding has limited structural interpretation. True structural slenderness depends on effective height (accounting for boundary conditions and top restraint quality), effective thickness (accounting for deterioration and eccentricity), and material properties (E-modulus degradation from moisture cycling), none of which were quantified in this study. The null correlation indicates that gross dimensional ratios alone do not predict observed degradation patterns in this dataset, but does not rule out the relevance of properly-defined structural slenderness. Future work incorporating structural assessment of restraint conditions, load eccentricity measurements, and material testing would be required to evaluate true slenderness effects.
...
Its statistical association with structural instability indicators suggests coating failure may serve as an observable indicator of structural deterioration, though the directionality of this relationship (whether coating failure exposes adobe to moisture that then causes structural damage, or whether structural movement causes coating cracks) cannot be determined from cross-sectional data.
...
\textbf{(4) Surface and Lintel Interaction (Factor 2):} Coat 1 Cracking and Lintel Deterioration group as Factor 2, indicating statistical association between coating damage and lintel decay, particularly around openings. The covariance pattern suggests these conditions may benefit from coordinated assessment and treatment, though the causal relationship remains unclear.
...
This study applied correlation analysis, Factor Analysis, and Elastic Net regression to 67 wall sections at Fort Union National Monument to identify patterns of damage co-occurrence and inform intervention prioritization. The multivariate analyses converged on three principal patterns of co-occurring damage: Sill Deterioration (Factor 1), Surface and Lintel Interaction (Factor 2), and Global Structural Instability (Factor 3), together explaining 52.5\% of variance in the degradation data. These clusters suggest distinct sets of conditions that tend to occur together, potentially indicating shared exposure factors or common underlying mechanisms requiring coordinated treatment strategies.

Elastic Net regression analysis of the relationship between individual condition scores and the composite Total Degradation Score identified Height Loss and Cap Deterioration as the most heavily weighted factors (standardized coefficients 43.0 and 54.5). However, because the Total Score is mathematically derived from these individual condition scores, the high $R^2$ (0.76) reflects the internal consistency of the scoring formula rather than independent predictive power. This analysis serves to empirically validate and quantify the implicit weights embedded in the assessment protocol, confirming that material loss and water protection dominate the composite rating. A supplementary analysis of gross dimensional ratios ($N=38$) demonstrated that dimensional ratios alone explain negligible variance in the composite scores ($r \approx 0$), confirming that condition metrics dominate the existing scoring system.

Treatment variables (bracing, historical interventions) showed negligible association with current condition scores, likely reflecting selection bias: treatments were applied reactively to already-degraded walls, and cross-sectional data cannot disentangle this confounding from treatment efficacy. Longitudinal monitoring with treatment timing data would be required to assess intervention effectiveness.

The intervention matrices translate statistical patterns and scoring weights into preservation-compatible action frameworks by multiplicatively combining condition importance (derived from factor variance and regression coefficients) with Secretary of Interior Standards compliance scores. Higher cell scores identify interventions addressing statistically-important conditions through preservation-appropriate means. Field validation remains necessary to confirm these analytically-derived priorities match practical effectiveness.

Transferability to other adobe sites depends on several factors. The damage pattern identification workflow and scoring weight recovery approach are generalizable; site-specific calibration would require adjustment for local climate, construction methods, and treatment history. Extensions incorporating temporal monitoring, climate scenario modeling, and cost data would strengthen decision-support utility. The limitations outlined in Section 6, particularly sample size and single-site scope, constrain extrapolation beyond FOUN without additional validation studies.
...
\includepdf[pages=1-14]{FOUN_RAS_Glossary.pdf}
...
\textbf{1. Data Exploration and Pattern Analysis} (\texttt{data\_exploration\_and\_pattern\_analysis.ipynb}): This notebook provides a step-by-step walkthrough of the analytical pipeline, focusing on two key objectives: identifying latent patterns of damage co-occurrence using Factor Analysis, and recovering the implicit weights of the scoring system using Elastic Net regression. Each code cell is annotated with explanations of statistical assumptions and interpretation guidelines, helping users distinguish between predictive modeling and weight recovery analysis.

\textbf{2. Expert-Statistical Intervention Framework} (\texttt{intervention\_matrix\_notebook.ipynb}): This notebook illustrates the process of translating statistical findings into actionable intervention matrices. It demonstrates how statistical importance (derived from Factor Analysis variance and recovered regression weights) is combined with preservation ethics (Secretary of Interior Standards) to generate priority scores. The notebook includes interactive modules for building intervention matrices and simulating budget allocation decisions.
