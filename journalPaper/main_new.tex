...
We proceeded with Factor Analysis as Bartlett's test strongly rejects variable independence and the extracted factors align with theoretically meaningful damage mechanisms.
% in case we want it later the values were ($\chi² = 376.04$, $df = 28$, $p < 0.001$)
...
The three extracted factors explained 65.1\% of the total variance, which is considered adequate for Exploratory Factor Analysis (typical threshold: $>60\%$ \cite[]{Hair2010}, with individual factor contributions of 26.1\% (Factor 1), 24.9\% (Factor 2), and 14.0\% (Factor 3). 
...
\multicolumn{2}{l}{\footnotesize \cite[]{dillon1984multivariate}.} \\
\end{tabular}
\end{table}
...
Coat 1 Cracking exhibits a Heywood case (communality slightly $>1.0$), a known phenomenon in Exploratory Factor Analysis with small samples that does not invalidate the overall structure but reinforces the exploratory interpretation of results \cite[]{DeWinter2009}.
The factor loadings are presented in Table \ref{tab:fa_loadings}.
...
This geometric-only model achieved CV R² = 0.29 (±0.15), RMSE = 10.22, MAE = 8.54, and out-of-bag R² = 0.34, representing reduced but non-trivial predictive performance compared to the full model (CV R² = 0.52).
...
\hl{should this be updated to 0-10 im not sure why its 0-6 right now}
