...
Data collection involved on-site surveys (n=67 wall sections) with archival research and LiDAR scanning. Factor Analysis revealed three distinct latent structures of degradation—Sill Deterioration, Surface/Lintel Interaction, and Structural Instability—together explaining 52.5\% of variance. 
Random Forest analysis identified Wall Height and Cap Deterioration as the strongest predictors of overall degradation, alongside Out-of-Plane movement. The prominence of Wall Height (permutation importance 0.24) highlights the critical role of geometric slenderness in adobe vulnerability, while the significance of Cap Deterioration (0.21) emphasizes the importance of water protection.
...
Limitations include small sample size (n=67) and single-site scope. The methodology offers a replicable framework for adobe assessment, validating the integration of geometric and condition data for risk stratification.
...
Additionally, \cite[]{kallas2024understanding} utilized Random Forest and other ML models to analyze structural vulnerability in unreinforced masonry buildings following the 2020 Beirut port explosion, identifying building geometry and age as key features shaping damage severity. 
Expanding on this, \cite[]{kallas2025kakhovka} applied similar techniques to assess masonry damage after the 2023 Kakhovka Dam breach in Ukraine, demonstrating the critical importance of contextual factors like duration of flooding and foundation type. 
...
Model performance metrics indicated limited predictive accuracy with cross-validation R² = 0.17 (±0.78), RMSE = 117.2, and MAE = 84.8. 
The training R² (0.96) substantially exceeded cross-validation R² (0.17), indicating significant overfitting as expected given the feature-to-sample ratio of approximately 1:2.4. 
To ensure the reliability of feature importance rankings despite the small dataset and overfitting, we computed permutation importance scores with 95\% confidence intervals via bootstrapping (1000 iterations). 

We note that while the cross-validated $R^2$ of 0.17 is low for a predictive model, it is not unexpected for complex field data derived from historic structures, where unmeasured variables (e.g., specific construction batches, micro-climate variations, exact maintenance dates) introduce substantial noise. 
In social and behavioral sciences, where human and environmental factors interact complexly, $R^2$ values in this range are often considered acceptable for identifying significant associations and trends, even if they lack high predictive precision \cite[]{cohen2013statistical, hair2011pls}.
For the purpose of this study—identifying relative risk factors to inform prioritization—the robust separation of top predictors (Height, Cap Deterioration) from noise variables is more critical than the ability to predict the exact degradation score.
...
The top predictors of degradation were Wall Height (importance = 0.17, permutation importance = 0.24 ± 0.03), Cap Deterioration (0.21, perm = 0.21 ± 0.03), and Out of Plane movement (0.18, perm = 0.20 ± 0.03) as shown in Figure \ref{fig:random_forest_new}. 
Structural Cracking also showed significant importance (0.16, perm = 0.15 ± 0.02).

These results represent a significant finding: Wall Height is the single strongest predictor of overall degradation, outranking even direct damage indicators like cracking. 
This confirms that geometric slenderness is a primary driver of vulnerability in these adobe structures.
Cap Deterioration's high ranking underscores the critical role of water protection at the wall crest.
The model's low cross-validated R² (0.17) reflects the high variability and noise inherent in field data from complex historic structures, but the clear separation of top predictors (Height, Cap Det, Out of Plane) from others provides a robust basis for prioritization.
Field managers can use these findings to prioritize interventions: tall walls and those with compromised caps require immediate attention, as these factors are most strongly associated with severe degradation.

\subsubsection{Supplemental Analysis: Geometric Factors Without Damage Confounding}

To isolate the predictive power of geometric and contextual features without damage indicator confounding, we conducted a supplemental Random Forest analysis predicting Total Score using only non-damage variables: Height, Foundation Height, Treatment, Bracing, Bracing Score, Point Cloud Mean, and Point Cloud Deviation.

This geometric-only model achieved poor predictive performance (CV R² < 0), indicating that geometric factors alone are insufficient to predict the complex degradation scores in this small dataset without the inclusion of direct damage indicators. 
However, the strong performance of Height in the full model (where it ranks as the top predictor) confirms its critical role.
Feature importance rankings in the geometric model (Figure \ref{fig:random_forest_geometric}) still highlight Height and Point Cloud Deviation as the most relevant geometric features.

\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\textwidth]{Images/feature_importance_geometric_only.png}
    \caption{Supplemental Random Forest Feature Importance: Geometric/Contextual Factors Only.}
    \label{fig:random_forest_geometric}
\end{figure}

Height emerged as the dominant predictor (MDI = 0.31, perm = 0.69 ± 0.07), followed by Point Cloud Deviation (MDI = 0.20, perm = 0.28 ± 0.03).
While the model's overall predictive power is low, the internal ranking confirms that among geometric variables, simple wall height and surface irregularity (Point Cloud Deviation) are the most significant risk factors.
...
None of the walls exceeded the critical h/t threshold of 10 typically associated with instability in unreinforced masonry, though the tallest walls (h/t = 8.33) approach this limit.
...
\textbf{(1) Structural Instability (Factor 3, RF rank 2-3):} This encompasses Structural Cracking  and Out of Plane movement, grouping into Factor 3 alongside Coat 2 Cracking. 
...
\textbf{(2) Surface Coating Degradation (Factor 3, RF rank 1):} Coat 2 Cracking emerged as the single strongest predictor, loading heavily on Factor 3. 
...
The statistical findings, including Factor Analysis loadings and Random Forest feature importance, were utilized as quantitative evidence to identify and validate the core set of NSCs relevant to degradation.
...
\begin{itemize}
    \vspace{-0.5em}
    \item Factor 3 explains 12.4\% of variance (Factor 1 explains maximum: 23.5\%)
    \item Highest loading in Factor 3: Coat 1 Cracking (0.60)
    \item Maximum loading across all factors: Sill 2 on Factor 1 (0.97)
    \item Variance contribution normalized: 12.4/23.5 = 0.528
    \item Loading strength normalized: 0.60/0.97 = 0.619
    \item Combined metric: (0.528 + 0.619)/2 = 0.574
    \item Priority score: 10 $\times$ 0.574 = 5.7, rounded to 6
    \item Cell score for mitigation: 6 (priority) $\times$ 6 (mitigation score) = 36
\end{itemize}

\textit{Note on Priority Score Adjustment:} While the worked example above yields a calculated priority of 6, the final priorities assigned in Tables \ref{tab:intervention_matrix_adobe} and \ref{tab:intervention_matrix_foundation} incorporate adjustment based on preservation practice considerations and NPS management priorities. Structural Instability receives priority 9 (vs. calculated 6) due to immediate structural safety concerns and cascading effects on other damage types. Sill Deterioration receives priority 8 (vs. calculated 10) to balance resource allocation. These adjustments ensure intervention priorities align with both statistical significance and operational preservation practice.
...
Additionally, this case study focuses exclusively on Fort Union National Monument. 
...
Furthermore, our data represent a single temporal snapshot, precluding analysis of degradation rates, treatment effectiveness over time, or causal inference about intervention impacts. 
...
Additionally, while we discuss slenderness, overturning moments, and bearing pressures qualitatively, finite element modeling could quantify stress distributions, factor-of-safety margins, and failure probabilities. 
...
Random Forest analysis identified Wall Height and Cap Deterioration as the strongest predictors of degradation (permutation importance 0.24 and 0.21), challenging the assumption that geometric factors are secondary to visible damage. 
While a supplemental geometric-only model lacked predictive power (R² < 0), the prominence of height in the full model confirms that slenderness is a fundamental driver of vulnerability, even when accounting for existing damage.
This suggests that while geometric data alone cannot predict the exact degradation score without condition data, tall walls inherently warrant higher prioritization for preventive maintenance.
