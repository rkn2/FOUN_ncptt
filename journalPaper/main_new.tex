...
Random Forest analysis identified Wall Height and Cap Deterioration as the strongest predictors of overall degradation, alongside Out-of-Plane movement. Wall Height (permutation importance 0.24) demonstrates that geometric slenderness governs adobe vulnerability, while Cap Deterioration (0.21) reflects the dependence on water protection at the wall crest.
...
\cite[]{kallas2024understanding} utilized Random Forest and other ML models to analyze structural vulnerability in unreinforced masonry buildings following the 2020 Beirut port explosion, identifying building geometry and age as key features shaping damage severity. 
...
We develop a data-driven approach for assessing structural vulnerability in adobe structures at Fort Union National Monument, integrating correlation analysis, Factor Analysis, and Random Forest regression to identify Necessary and Sufficient Conditions (NSCs) affecting structural behavior. 
...
For this study—identifying relative risk factors to inform prioritization—the statistical separation of top predictors (Height, Cap Deterioration) from noise variables matters more than precise prediction of the degradation score.
...
Cap Deterioration's high ranking reflects the dependence on water protection at the wall crest.
The model's low cross-validated R² (0.17) reflects the high variability and noise inherent in field data from complex historic structures, but the clear separation of top predictors (Height, Cap Det, Out of Plane) from others provides a statistical basis for prioritization.
...
The strong performance of Height in the full model (where it ranks as the top predictor) confirms its governing influence on degradation.
Feature importance rankings in the geometric model (Figure \ref{fig:random_forest_geometric}) identify Height and Point Cloud Deviation as the dominant geometric features.
